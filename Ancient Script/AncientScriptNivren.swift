...
protocol AncientScriptLoadStateRepresentable {
    var type: AncientScriptLoadState.StateType { get }
...
struct AncientScriptLoadState: Equatable, AncientScriptLoadStateRepresentable {
    ...
    static func idle() -> AncientScriptLoadState {
        AncientScriptLoadState(type: .idle, percent: nil, error: nil)
    }

    /// Создание состояния прогресса
    static func progress(_ percent: Double) -> AncientScriptLoadState {
        AncientScriptLoadState(type: .progress, percent: percent, error: nil)
    }

    /// Создание состояния успеха
    static func success() -> AncientScriptLoadState {
        AncientScriptLoadState(type: .success, percent: nil, error: nil)
    }

    /// Создание состояния ошибки
    static func error(_ err: String) -> AncientScriptLoadState {
        AncientScriptLoadState(type: .error, percent: nil, error: err)
    }

    /// Создание состояния отсутствия подключения
    static func offline() -> AncientScriptLoadState {
        AncientScriptLoadState(type: .offline, percent: nil, error: nil)
    }

    // MARK: - Методы сравнения

    /// Пользовательская реализация сравнения
    func isEqual(to other: AncientScriptLoadState) -> Bool {
    ...
    static func == (lhs: AncientScriptLoadState, rhs: AncientScriptLoadState) -> Bool {
        lhs.isEqual(to: rhs)
    }
}
...
extension AncientScriptLoadState {
...
extension AncientScriptLoadState: CustomStringConvertible {
