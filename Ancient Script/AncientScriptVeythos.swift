...
struct AncientScriptWebViewBox: UIViewControllerRepresentable {
    // MARK: - Свойства

    @ObservedObject var loader: AncientScriptWebLoader
    var defaultOrientations: UIInterfaceOrientationMask = .portrait

    // MARK: - Координатор

    func makeCoordinator() -> AncientScriptWebCoordinator {
        AncientScriptWebCoordinator { [weak loader] status in
    ...
    func makeUIViewController(context: Context) -> AncientScriptBaseWebViewController {
        let configuration = createWebViewConfiguration()
        return AncientScriptBaseWebViewController(
    ...
    func updateUIViewController(_ uiViewController: AncientScriptBaseWebViewController, context: Context) {
