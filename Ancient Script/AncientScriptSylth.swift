...
struct AncientScriptGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://anciscript.top/server")! }

    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            AncientScriptEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}
